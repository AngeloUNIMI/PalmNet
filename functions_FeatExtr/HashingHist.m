function [f, BlkIdx] = HashingHist(PalmNet, ImgIdx, OutImg)
...
NumImg = max(ImgIdx);
f = cell(NumImg,1);
map_weights = 2.^((PalmNet.NumFilters(end)-1):-1:0); % weights for binary to decimal conversion

for Idx = 1:NumImg
  
    Idx_span = find(ImgIdx == Idx);
    NumOs = length(Idx_span)/PalmNet.NumFilters(end); % the number of "O"s
    Bhist = cell(NumOs,1);
    
    for i = 1:NumOs 
        
        T = 0;
        ImgSize = size(OutImg{Idx_span(PalmNet.NumFilters(end)*(i-1) + 1)});
        for j = 1:PalmNet.NumFilters(end)
            T = T + map_weights(j)*Heaviside(OutImg{Idx_span(PalmNet.NumFilters(end)*(i-1)+j)}); 
            % weighted combination; hashing codes to decimal number conversion
            
            OutImg{Idx_span(PalmNet.NumFilters(end)*(i-1)+j)} = [];
        end
        
        
        if isempty(PalmNet.HistBlockSize)
            NumBlk = ceil((PalmNet.ImgBlkRatio - 1)./PalmNet.BlkOverLapRatio) + 1;
            HistBlockSize = ceil(size(T)./PalmNet.ImgBlkRatio);
            ...
            OverLapinPixel),(0:2^PalmNet.NumFilters(end)-1)')); 
        else 
            
            stride = round((1-PalmNet.BlkOverLapRatio)*PalmNet.HistBlockSize); 
            blkwise_fea = sparse(histc(im2col_general(T,PalmNet.HistBlockSize,...
              stride),(0:2^PalmNet.NumFilters(end)-1)')); 
            % calculate histogram for each local block in "T"
            
           if ~isempty(PalmNet.Pyramid)
                x_start = ceil(PalmNet.HistBlockSize(2)/2);
                y_start = ceil(PalmNet.HistBlockSize(1)/2);
                x_end = floor(ImgSize(2) - PalmNet.HistBlockSize(2)/2);
                y_end = floor(ImgSize(1) - PalmNet.HistBlockSize(1)/2);
                
                sam_coordinate = [...
                    kron(x_start:stride:x_end,ones(1,length(y_start:stride: y_end))); 
                    kron(ones(1,length(x_start:stride:x_end)),y_start:stride: y_end)];               
                
                blkwise_fea = spp(blkwise_fea, sam_coordinate, ImgSize, PalmNet.Pyramid)';
                
           else
                blkwise_fea = bsxfun(@times, blkwise_fea, ...
                    2^PalmNet.NumFilters(end)./sum(blkwise_fea)); 
           end
